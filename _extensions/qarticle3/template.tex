...
{$by-author.name.literal$\textsuperscript{$for(by-author.affiliations)$$it.letter$$endfor$$if(remove-emails)$$else$,$by-author.number$$endif$$sep$$endfor$
$for(by-author/last)$
{ and $by-author.name.literal$\textsuperscript{$for(by-author.affiliations)$$it.letter$$endfor$$if(remove-emails)$$else$,$by-author.number$$endif$}}$endfor$
$else$
${by-author.name/last}
$endif$ \vskip 24.5pt
$endif$
...
$for(affiliations)$
\emph{$affiliations.address/nowrap$}\par
$endfor$
$else$
\setlength\parskip{-.1em}
		$for(affiliations)$
